...
//-----------------------------------------------------------------------
//Insertion from task 37608
//Need to show/hide password
var show = false;

// console.log("showVariable is ",showVariable);
document.getElementById('showhidepass').innerHTML = showVariable;
function showHidePass() {
    if (!show) {
        document.getElementById('password').type = 'text';
        document.getElementById('showhidepass').innerHTML = hideVariable;
    }
    else {
...
(function($){
	...
	var oldBrowsers = (/(?=.*opera)(?=.*version\/11)|(?=.*opera)(?=.*version\/10)/i.test(navigator.userAgent));
	// Redirect to HTTPS if IE <=9
	if (window.location.protocol !== "https:" && (SITE.isIeLte9 || oldBrowsers ))
...
$(function(){
    ...
    *----------------------------------------------------
	* MODEL OF LOGIN FORM
	* Includes rules, validators, list of required fields
	* extend backbone
	* Use backbone-validation.js
	* @MODEL
	*----------------------------------------------------
	*/
	var AuthModel = Backbone.Ribs.Model.extend({
		...
		getParams: '', //(window.suffix!=='') ? '?'+window.suffix : '',

		url:{
			info: 'https://' + location.host + '/authentication/api?action=info',
			form: 'https://' + location.host + '/authentication/api?action=login',
			gotoMy: SITE.my
		},

		redirect: function(){

			if('undefined' !== typeof(Drupal.settings.fxtm_core) && 'undefined' !== typeof(Drupal.settings.fxtm_core.webinar_type))
			    window.location.href = this.url.gotoMy+'/'+Drupal.settings.fxtm_core.webinar_type;

			else if(window.myfxtmRedirect)
			 	window.location.href = window.myfxtmRedirect;
			else
			    window.location.href = this.url.gotoMy + this.getParams;

		},

		send: function( data, url, f ){
	        ...
	        $.ajax({
	            ...
	            data: data,
	            dataType: 'json',
	            beforeSend: function(){_.isFunction( f.beforeSend ) && f.beforeSend()},
	            success: function(e){_.isFunction( f.success ) && f.success(e)},
	            error: function(e){_.isFunction( f.error ) && f.error(e)},
	            complete: function(xhr){_.isFunction( f.complete ) && f.complete()},
	            xhrFields: {
	...
	*/
	...
	var AuthForm = Backbone.Ribs.View.extend({
		...
		initialize: function () {
			...
			this.model.send('info', this.model.url.info, {

				success: function(msg){
					if( !_.isEmpty(msg) && !_.isUndefined(msg.id) ) {
						self.model.redirect();
					}
					else {
						if('undefined' != typeof(msg.country_mismatch) && SITE.version != msg.country_mismatch) {
							self.showModal(msg.country_mismatch, msg, 'initialize');
						}
					}
			...
			this.model.set('login',this.$el.find('input#login'));
			this.model.set('password',this.$el.find('input#password'));

			self.checkFields();

			//this.model.url.form = this.$el.attr('action'); // Now we use api url instead form action
		...
		events: {
		  ...
		  'focusout input': function(e){
		  		...
		  		e.preventDefault();
		  	  	this.checkFields( $(e.currentTarget) );

		  	}

		},

        showModal:function(region, msg, action){

            var self = this;

            $('#modal').html(_.template($('#login_modal').html(),{'region':region})).modal({backdrop: 'static', keyboard: false});
...
;           });
		...
		renderErrors: function(errors) {
			...
			$('#login-container').find('.help-block').show();

		},

		// Enable or disable AUTH button
		allowAuth: function ( allow, o ){

			if( _.isUndefined(o) ) o = {};
			var add = o.add || '';
			var rem = o.rem || '';
			$('#auth').attr('disabled', !allow).addClass(add).removeClass(rem);

		},

		checkFields: function(x){

			if(!_.isUndefined(x)) this.model.set(x.attr('name'),x.val() );

		  	this.allowAuth( this.model.get('login') != '' && this.model.get('password') != '' );
		...
		auth: function () {
	     ...
	     var self = this;
	     this.model.send( $.extend({hits: window.getUserPath()}, this.$el.serializeObject()), this.model.url.form, {

	     	beforeSend: function(){
	     		self.allowAuth( false, {add: 'loading'} );
	     		$('#login-container').find('.help-block').hide();
	     	},

	     	error: function(e){
	     		self.allowAuth( true, {rem: 'loading'} );
	     	},

	      	success: function(msg){

	      		if( msg.status ){
					window.clearUserPath();
					if(msg.status == 'maintenance') {
						$('#login-container').find('.help-block').html(Drupal.t("We apologize but we were unable to log you in. Please try again.")).show();
						self.allowAuth( true, {rem: 'loading'} );
					}else{
						if('undefined' != typeof(msg.country_mismatch) && SITE.version != msg.country_mismatch) {
							self.showModal(msg.country_mismatch, msg, 'auth');
						}
						else {
							$.utils.setAuthCookie(msg.sid);
							self.model.redirect();
	...
	var view = new AuthForm({
	  el: 'form#authorizing-form',
	  model: new AuthModel()
	});
