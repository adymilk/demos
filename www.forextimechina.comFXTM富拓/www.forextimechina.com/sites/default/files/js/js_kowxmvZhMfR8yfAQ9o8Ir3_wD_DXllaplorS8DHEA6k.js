...
(function($, Backbone) {
    ...
    var format = function(number, bigEnd, xauExclusion) {
        var res=number+'';
        res = (res.length>7)? res.slice(0,7) : res;
        if(res.replace(',','').length<7)
            while(res.replace(',','').length<7)
                res+='0';

        // 3 last - big
        if(bigEnd) {
            var resStart = res.slice(0, res.length - 3);
            var resSfx = res.slice(-3);
            if(!xauExclusion) {
                var resEnd = resSfx.slice(-1);
                resSfx = resSfx.slice(0, 2);
            }else{
                var resEnd = '';
            }
            res = resStart + '<span>' + resSfx + '</span>' + resEnd;
        }
        return res;
    };
    ...
    var Rate = Backbone.Model.extend({
        ...
        RateView = Backbone.Ribs.View.extend({
            tagName: 'tr',
            initialize: function() {
                this.$el.html('<td class="widget-rates-td-instr">' + this.model.get('id') + '</td><td class="widget-rates-td-rate '+this.model.get('direction')+'"> </span><span class="widget-rates-arrow"></span> <span class="widget-rates-value">' + format(this.model.get('bid'), 1, this.model.get('id').toLowerCase()=='xauusd') + '/' + format(this.model.get('ask'), 1, this.model.get('id').toLowerCase()=='xauusd') + '</td>');
            }
        });

    var RatesView = Backbone.Ribs.View.extend({
        bindings: {
            '.rates tbody': {
                collection: {
                    col: 'rates',
        ...
        initialize: function(opts) {
            ...
            this.rates.set(new Rate({id: "EURUSD", ask: 1, bid: 10, direction: 'up'}));
            this.getRates();
        },
        getRates: function(){
            var self = this;
            var rateLink = {
                fiats: '/informers/rates/full',
                crypto: '/informers/rates/crypto',
                mix: '/informers/rates/fullandcrypto'
            };
            $.getJSON( window.currencyType && rateLink[window.currencyType] || rateLink.fiats)

                .done(function( json ) {

                    //self.rates.reset();
                    _.each(json, function(rate) {
                        if(!_.include(window.ratePairs, rate.symbol.toLowerCase())) return;

                        var old = self.rates.get(rate.symbol);

                        if(old){
                            var direction = (rate.bid < old.get('bid')) ? 'down':'up';
                            self.rates.remove(old);
                            self.rates.add(new Rate({id: rate.symbol, ask: rate.ask, bid: rate.bid, direction: direction}));
                        }else{
                            self.rates.add(new Rate({id: rate.symbol, ask: rate.ask, bid: rate.bid, direction: 'up'}));
                        }
    ...
    $(function() {
        ...
        var widget = $('.widget-live-currency-rates');
        new widgetTools(widget);
